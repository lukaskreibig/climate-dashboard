...
/**
 * Arctic Albedo Explainer – v4
 * -------------------------------------------------------------
 * Stage 1  Ice‑dominated   →  incoming ray strikes ice, bright mirror‑like reflected ray
 * Stage 2  Shrinking ice   →  same geometry, reflection weakened, icebergs drifting
 * Stage 3  Open water      →  incoming ray still diagonal (per user request), no reflection, ocean heat‑up
 */
const WhyArcticExplainer: React.FC<WhyArcticExplainerProps> = ({ apiRef }) => {
  const [stage, setStage] = useState(1);

  /* Allow parent to imperatively change the stage */
  useImperativeHandle(apiRef, () => ({ showStage: (s: number) => setStage(s) }), []);

  // 🎨 tweak‑friendly palette
  const waterColor = '#0EA5E9';
  const sunColor   = '#FDE047';

  // ─────────────────────────  RAY GEOMETRY  ─────────────────────────
  // A single constant angle requested by user (≈ 25° clockwise from vertical)
  const rayRotationDeg = 25; // works for all three stages

  // Reflection is present only while ice exists (stages 1–2)
  const showReflection = stage < 3;
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-b from-sky-200 via-sky-100 to-blue-100 relative overflow-hidden text-[14px]">
        ...
        {stage >= 1 && (
          <motion.div
            ...
            style={{
              width        : '14px',
              height       : '500px',
              right        : '105px',   /* same X as screenshots */
              top          : '120px',  /* under the sun */
              borderRadius : '7px',
              ...
              rotate:   rayRotationDeg
            }}
            // initial={{ opacity: 0, scaleY: 0, rotate: rayRotationDeg }}
            // animate={{ opacity: 1, scaleY: [0, 1, 1.05], rotate: rayRotationDeg }}
        ...
        {showReflection && (
          <motion.div
            ...
            style={{
              width        : '14px',
              height       : '260px',
              right        : '260px',          // anchor directly beneath the incoming ray
              /* Contact point ≈ top of waterline (40% height) plus 4% to land exactly on the ice
                 This keeps the bottom of the reflected ray glued to the ice surface. */
              bottom       : '40%',
              borderRadius : '7px',
            ...
            // initial={{ opacity: 0, scaleY: 0, rotate: -rayRotationDeg }}
            // animate={{ opacity: [0, 1, 0.8, 1], scaleY: [0, 1.05, 1], rotate: -rayRotationDeg }}
            exit={{ opacity: 0, scaleY: 0 }}
            transition={{ duration: 1.5, ease: 'easeInOut' }}
          />
        )}
      ...
      <div className="absolute bottom-0 left-0 w-full h-2/5" style={{ backgroundColor: waterColor }}>
        ...
        {/* Floating ice‑bergs (return of the break‑offs!) */}
        <AnimatePresence>
          {stage >= 2 && (
            <motion.div key="floating-ice" className="absolute inset-0 pointer-events-none">
              {/* Large chunk */}
              <motion.div
                ...
                transition={{
                  ...
                  opacity: { duration: stage === 3 ? 2 : 1 },
                  scale  : { duration: stage === 3 ? 2 : 1 },
              ...
              <motion.div
                ...
                transition={{
                  x      : { duration: 3.2, ease: 'easeOut' },
                  opacity: { duration: stage === 3 ? 1.8 : 1.2 },
                  scale  : { duration: stage === 3 ? 1.8 : 1.2 },
                ...
                <>
                  {[...Array(2)].map((_, i) => (
                    <motion.div
                      ...
                      transition={{
                        x      : { duration: 2.6 + i * 0.4, ease: 'easeOut' },
      ...
      <AnimatePresence>
        {stage === 3 && (
          ...
          >
            <div className="bg-red-500 text-white px-6 py-3 rounded-full font-bold text-lg shadow-lg">🌡️ +2°C Faster</div>
            <motion.div
      ...
      {/* Labels with updated positioning */}
      <AnimatePresence>
        <motion.div
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex space-x-3">
        {[1, 2, 3].map(step => (
          <div
            ...
            className={`w-3 h-3 rounded-full transition-all duration-500 ${stage >= step ? 'bg-sky-600 scale-110' : 'bg-sky-300 bg-opacity-50'}`}
          />
        ))}
      </div>
    </div>
