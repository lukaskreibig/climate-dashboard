...
(* The Sentinel‑2 UNet pipeline is maintained in the companion repository [uummannaq-ice-from-space](https://github.com/lukaskreibig/uummannaq-ice-from-space).)
```
...
2. **`update_fjord_data.py`**
   - Reads processed Sentinel‑2 segmentation output (exported by the external repository).
   - Derives season bands, spring anomalies, mean fractions, and freeze/breakup dates.
   - Upserts into `fjord_*` tables in PostgreSQL.

3. **Sentinel‑2 segmentation** (hosted in [uummannaq-ice-from-space](https://github.com/lukaskreibig/uummannaq-ice-from-space))
